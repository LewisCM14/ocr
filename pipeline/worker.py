...
def claim_batch(
    ...
    with engine.begin() as conn:
        ...
        if dialect == "sqlite":
            ...
            def get_id(row):
                return row if isinstance(row, tuple) else row.id
            def get_fp(row):
                return row if isinstance(row, tuple) else row.file_path
            def get_fn(row):
                return row if isinstance(row, tuple) else row.file_name
            ...
            return [{"id": get_id(row), "file_path": get_fp(row), "file_name": get_fn(row)} for row in rows]
        else:
            rows = conn.execute(
                text(f"""
